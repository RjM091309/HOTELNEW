...
$(document).ready(function () {
    // Long-term stay page always uses 'all' scope so groups are represented once
    const scope = 'all';
    ...
    let table = $('#booking_tbl').DataTable({
        ...
        ajax: {
            ...
            dataSrc: function (json) {
                // Map first, then filter rows flagged as Long-Term Stay only
                ...
                return mapped.filter(row => String(row.IsLongTermStay) === '1');
            },
        },
        autoWidth: false,
        columnDefs: [
        ...
        order: [[0, 'desc']],
        language: { emptyTable: 'No long-term stay bookings found.' }
    });
    ...
    // Date filter buttons - still fetch by date but keep client-side Long-Term filter
