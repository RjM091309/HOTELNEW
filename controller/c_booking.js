...
class BookingController {
  ...
  static async addBooking(req, res) {
    try {
      ...
      // PUAP: flight number is required for whichever of Pick-up/Drop-off is selected
      if (pickupServiceId && (!flightNumber || !String(flightNumber).trim())) {
        return res.status(400).json({ success: false, message: 'Flight Number is required for Pick-up.' });
      }
      if (dropoffServiceId && (!dropoffFlightNumber || !String(dropoffFlightNumber).trim())) {
        return res.status(400).json({ success: false, message: 'Flight Number is required for Drop-off.' });
      }
      ...
      const result = await BookingModel.addBooking({
        ...
        bookingRemarks,
        agencyID,
        agencyPayer: (finalBookingRoute === 'agency' && agencyPayer) ? agencyPayer : null,
        guestID,
        guestType,
        guestLevel,
        nationality: nationality && String(nationality).trim() !== '' ? String(nationality).trim() : null,
        breakfastAdultQty,
        breakfastAdultPrice,
        breakfastAdultId,
        breakfastKidQty,
  ...
  static async getRangeAvailabilityCounts(req, res) {
    try {
      const { startDate, endDate, checkInStatus, checkOutStatus, floorNumber, category, breakfast } = req.query;
